...
// 在场景中添加坐标轴
    ...
    function initTexture(url) {
        return THREE.ImageUtils.loadTexture(url, {}, function () {
            //renderer.render(scene, camera);
        });
    }
    ...
    container_shift = function (row, column, layer, nrow, ncolumn, nlayer) {
        ...
        for (var x in group.children) {
            if (group.children[x].name === name) {
                ...
                con.name = nname;
                con.position.x += (ncolumn - column) * unitLen;
                con.position.z -= (nrow - row) * unitLen;
                con.position.y += (nlayer - layer) * containerHeight;
                break;
            }
        }
    };
